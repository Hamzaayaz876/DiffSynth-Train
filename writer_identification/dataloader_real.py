...
class DatasetFromFolder(data.Dataset):
        def __init__(self,dataset,foldername,labelfolder,imgtype='png',scale_size=(64,128),
                ...
                for imgfile in self.imglist:
                        try:
                                Image.open(os.path.join(self.folder, imgfile)).verify()  # checks if image is valid
                                valid_imglist.append(imgfile)
                        except (IOError, OSError, ValueError):
                                print(f"Warning: skipping corrupted image {imgfile}")

                self.imglist = valid_imglist
        ...
        def resize(self,image):
                ...
                imre = Image.fromarray(image).resize((nw, nh), Image.BILINEAR)
                imre = np.array(imre)
        ...
        def __getitem__(self,index):
                ...
                image = np.array(
                        Image.open(self.folder + imgfile).convert('L')
                        )
